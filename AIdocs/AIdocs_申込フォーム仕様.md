...
| usage_url_domain | VARCHAR(255) | NOT NULL | ご利用URL・ドメイン |
| import_from_trial | BOOLEAN | DEFAULT FALSE | 体験版からのインポートを希望する |
...
- 電話番号（必須）※数字とハイフン
  ...
  - **全角数字→半角数字変換**: 全角数字（０-９）で入力された場合は自動的に半角数字（0-9）に変換
- 郵便番号 ※7桁（ハイフン有無可）
...
#### 1.2 ご利用情報
- ご利用URL・ドメイン（必須）※
  - **全角英数字→半角英数字変換**: 全角英数字（Ａ-Ｚ、ａ-ｚ、０-９）で入力された場合は自動的に半角英数字（A-Z、a-z、0-9）に変換
- 体験版からのインポートを希望する（任意）※チェックボックス
...
- **カード情報入力**:
  - カード番号（4桁×4フィールド、自動フォーカス移動）
    - **全角数字→半角数字変換**: 全角数字（０-９）で入力された場合は自動的に半角数字（0-9）に変換
    - 4桁入力で次のフィールドに自動フォーカス移動
    - Backspaceで前のフィールドに戻る（空の場合）
  - 有効期限（月・年）
    - 有効期限（年）: **全角数字→半角数字変換**: 全角数字（０-９）で入力された場合は自動的に半角数字（0-9）に変換（2桁または4桁で入力）
  - カード名義
  - セキュリティコード（任意）
    - **全角数字→半角数字変換**: 全角数字（０-９）で入力された場合は自動的に半角数字（0-9）に変換（3桁または4桁で入力）
...
### 機能追加候補
1. ~~メール通知（申込完了、決済完了）~~ ✅ 実装済み（返信メール設定機能）
2. 契約更新・解約機能
3. 請求書発行
4. 利用状況レポート
5. プラン変更機能
...
| 2026-01-30 | AI | 申込者への返信メール機能を追加（詳細は「返信メール設定仕様.md」参照） |
