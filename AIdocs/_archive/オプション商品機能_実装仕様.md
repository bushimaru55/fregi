...
- **オプション商品**: `products`テーブル（`type='option'`）
  - `billing_type`: `one_time`（一回限り）または `monthly`（月額課金）を管理画面上で設定・保存可能（ベース製品同様）。決済API連携は行わない。
...
- `billing_type`: VARCHAR(20) DEFAULT 'one_time' - 決済タイプ（オプション製品で利用。one_time / monthly。管理画面での設定・保存のみ。決済API連携なし）
...
- アクセサ: `formatted_price`, `type_label`, `billing_type_label`（一回限り/月額課金表示用）
- `billing_type`: オプション製品の課金種別（one_time / monthly）。管理画面で設定・保存。決済API連携は行わない。
...
### 管理画面: 契約編集（admin/contracts/{id}/edit）

**追加（2026-01-30）:**
- オプション商品セクションを表示。契約の契約プランに紐づくオプション商品一覧をチェックボックスで表示する。
- 現在の契約明細（contract_items）に含まれるオプション商品を選択済みで表示。
- チェックの有無を変更して更新すると、契約明細のオプション行が同期される（追加・削除のみ。ベース商品の明細は変更しない）。
- 許可されるオプションは「その契約の契約プランに紐づくオプション商品」のみ。それ以外のIDは無視する。
...
**新規イベントタイプ（決済連携に応じて定義）:**
- 月額商品・買い切り商品それぞれのオーソリリクエスト／レスポンス／成功／失敗を記録するイベントタイプを、実装する決済方式（ROBOT PAYMENT 等）に合わせて定義する。
...
- ただし、決済連携先側では月額商品の顧客登録が完了している場合がある
- 警告ログに記録される
- **手動対応が必要な場合がある**（決済連携先でのキャンセル処理）
...
6. `app/Http/Controllers/Admin/ContractController.php` - `edit()`でオプション商品一覧・選択中IDを渡す。`update()`で`option_product_ids`を検証し契約明細を同期（2026-01-30）
7. `resources/views/admin/contracts/edit.blade.php` - オプション商品チェックボックスセクション追加（2026-01-30）

---

## 変更履歴

- **2026-01-30**: オプション製品に1回限り・月額課金対応を追加。`products.billing_type` カラム追加。管理画面の商品マスタでオプション製品に決済タイプを設定・保存可能に。決済API連携は行わない。マイグレーション: `2026_01_30_100006_add_billing_type_to_products_table.php`
- **2026-01-30**: 管理画面の契約編集画面にオプション商品の有無をチェックボックスで編集する機能を追加。契約プランに紐づくオプション商品一覧を表示し、チェックの変更で契約明細（contract_items）のオプション行を同期。
