...
#### 2. 環境判定ロジック
- **実装箇所**: `ContractController::store()`, `ReturnController::handle()`, 決済連携サービス（ROBOT PAYMENT 等）
- **判定方法**: 決済連携先のテスト/本番は実装する決済方式に応じて制御する
...
- **確認事項**:
  ...
  - （旧決済用の秘密鍵項目は削除済み。決済は今後追加される仕様書に従う）
...
- **確認事項**:
  - 本番環境の決済連携（ROBOT PAYMENT 等）の設定が登録されているか
