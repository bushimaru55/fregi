...
- メッセージ例: `550 5.1.1 <xxx@example.com>: Recipient address rejected: User unknown in virtual mailbox table`
- これは**アプリのバグではなく、SMTP サーバー（MAIL_* で接続しているサーバー）が受信者を拒否している**状態です。
- **原因**: 送信先に設定したアドレスが、そのメールサーバーの virtual mailbox table に存在しない（メールボックスが未作成／別サーバー管轄など）。
- **対処**:
  1. メールサーバー側で当該アドレスを virtual mailbox に追加する（本番でそのアドレスを使う場合）、または
  2. 送信先メールアドレスを、**現在の MAIL_* サーバーが受信可能なアドレス**に変更する（検証・ローカルではテスト用アドレスなど）。

## 変更履歴

- 2026-01-30: 複数メールアドレス対応を追加（従来は単一のみ）。550「User unknown in virtual mailbox table」の説明を追加。
