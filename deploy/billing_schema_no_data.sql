...
CREATE TABLE `contract_plans` (
  ...
  `billing_type` enum('one_time','monthly','yearly') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'one_time' COMMENT '決済タイプ（one_time: 一回限り, monthly: 月額課金, yearly: 年額課金）',
  `payment_collection_method` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'card' COMMENT '回収方法（card: クレジット, bank_transfer: 請求書払い・銀行振込）',
...
CREATE TABLE `contracts` (
  ...
  `payment_collection_method` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'card' COMMENT '申込時点の回収方法（card / bank_transfer）',
